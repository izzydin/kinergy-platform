...
| **Bounded Context**            | DDD Architecture     | Explicit boundary within which a domain model applies (`identity`, `client`, `scheduling`, `kinesiology`, `gym`, `resources`).                                                         |
...
| **Inventory Item**             | Consumable Inventory | Distinct consumable supply tracked by a unique SKU, unit of measure, and continuous fungible stock balance ([Vocabulary](./business/resources-vocabulary.md)).                         |
| **Stock Movement**             | Consumable Inventory | Immutable append-only ledger transaction recording quantity delta, post-movement balance, movement type, cost, and actor.                                                              |
| **Quantity on Hand**           | Consumable Inventory | Authoritative real-time physical balance of inventory available; guaranteed non-negative (`quantityOnHand >= 0` `[INV-INV-2]`).                                                        |
| **Minimum Stock**              | Consumable Inventory | Non-negative threshold below which automated replenishment alerts and low-stock indicators trigger.                                                                                    |
| **Stock Movement Type**        | Consumable Inventory | Operational classification: `PURCHASE` (+), `SALE` (-), `CONSUMPTION` (-), `ADJUSTMENT_IN` (+), `ADJUSTMENT_OUT` (-), `CORRECTION` (var), `SCRAP` (-).                                 |
| **Fixed Asset**                | Fixed Assets         | Non-fungible capital item of durable property tracked by unique `assetTag`, serial number, location, and operational state machine. Distinct from consumables.                         |
| **Asset Status**               | Fixed Assets         | Primary operational state enum: `ACTIVE`, `UNDER_MAINTENANCE`, `DAMAGED`, `RETIRED`, `SOLD` (Terminal State `[AST-INV-1]`).                                                            |
| **Asset Condition**            | Fixed Assets         | Severity-ranked physical health rating from 1 (`EXCELLENT`) to 5 (`OUT_OF_SERVICE`).                                                                                                   |
| **Asset Location**             | Fixed Assets         | Informational value object capturing physical placement (`facilityId`, `roomId`, `zone`). Operates independently from room bookings.                                                   |
| **Asset Maintenance Record**   | Fixed Assets         | Immutable servicing log recording service date, description, expenditure, vendor, and technician details.                                                                              |
| **Resource Valuation**         | Resources Overview   | Synthesized capital valuation combining consumable stock valuation ($\sum Q \times \text{cost}$) and fixed asset book value (`currentEstimatedValueAmount`).                           |
