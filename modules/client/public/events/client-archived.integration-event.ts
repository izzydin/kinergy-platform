...
/**
 * @public
 * Integration event published when a client profile is archived (soft-deleted).
 *
 * Consuming bounded contexts subscribe to this event to react to client archival —
 * e.g. cancelling future appointments, suspending memberships, or blocking POS access.
 *
 * @remarks
 * - This is an **integration event contract** — it crosses bounded-context boundaries.
 * - It is distinct from the internal `ClientArchivedEvent` domain event.
...
export class ClientArchivedIntegrationEvent {
  ...
  /** UUID of the client that was archived. */
