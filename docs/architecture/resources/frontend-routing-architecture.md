...
| `/resources/overview`          | `ResourceOverviewPage`    | Synthesized executive cockpit: Combined Resource Value, Consumable Inventory working capital & low-stock alerts, and Fixed Assets carrying values & operational lifecycle status distribution ([ADR-0102](./adr/0102-resource-overview-synthesized-read-query-architecture.md)). | `inventory.read` + `assets.read` + `billing.read` (or `ADMIN`, `SUPER_ADMIN`, `OWNER`) | `MainLayout` |
