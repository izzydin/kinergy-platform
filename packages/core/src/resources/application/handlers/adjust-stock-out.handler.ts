...
/**
 * Use case handler orchestrating negative manual stock adjustments (shrinkage, damage, audit deficit).
    ...
    try {
      ...
      if (!reason || reason.length < 3) {
        return ApplicationResult.fail(
          'A valid reason (minimum 3 characters) is required for stock adjustment out.',
        );
      }
      ...
      const message = err instanceof Error ? err.message : 'Failed to adjust stock out.';
