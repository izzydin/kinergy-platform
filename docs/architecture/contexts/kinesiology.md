...
| **`updateNotes(notes, clock?)`** | `SCHEDULED` or `IN_PROGRESS`  | State unchanged, `notes` updated | Throws `Error` if `notes` is null/undefined or if invoked on a session in `COMPLETED`, `CANCELLED`, or `NO_SHOW` terminal status.                                                 |
