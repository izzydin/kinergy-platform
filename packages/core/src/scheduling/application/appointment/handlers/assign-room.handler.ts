...
/**
 * CQRS Command Handler executing room reassignment workflows.
 */
export class AssignRoomHandler implements CommandHandler<
  AssignRoomCommand,
...
> {
  constructor(
    ...
    private readonly roomAvailabilitySpec: RoomAvailabilitySpecification,
    private readonly clock: Clock,
  ) {}

  /** Executes room reassignment with suitability and conflict checks */
  public async execute(command: AssignRoomCommand): Promise<ApplicationResult<AppointmentDTO>> {
    try {
      const { input } = command;
      ...
      if (
        ...
        return ApplicationResult.fail(
          `Cannot assign room to appointment in terminal '${appointment.status}' status.`,
        );
      }

      // Check room specifications (capacity, features, status)
      const roomId = RoomId.create(input.newRoomId);
      const room = await this.roomRepository.findById(roomId);
      if (room) {
        const isSuitable = this.roomAvailabilitySpec.isSatisfiedBy({
          room,
        ...
        if (!isSuitable) {
          return ApplicationResult.fail(
            `Room '${input.newRoomId}' does not satisfy availability or required specifications.`,
          );
        }
      }
