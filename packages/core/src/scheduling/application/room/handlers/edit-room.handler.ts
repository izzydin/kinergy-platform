...
export class EditRoomHandler implements CommandHandler<
  EditRoomCommand,
  ApplicationResult<RoomDTO>
> {
  constructor(private readonly roomRepository: RoomRepository) {}

  public async execute(command: EditRoomCommand): Promise<ApplicationResult<RoomDTO>> {
    try {
      const { input } = command;
      ...
      const message = error instanceof Error ? error.message : 'Failed to edit room.';
