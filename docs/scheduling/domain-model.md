...
Governs appointment lifecycle mutations (`SCHEDULED`, `CONFIRMED`, `CHECKED_IN`, `IN_PROGRESS`, `COMPLETED`, `CANCELLED`, `NO_SHOW`).

- **Key Methods:** `confirm()`, `checkIn()`, `start()`, `complete()`, `cancel()`, `markNoShow()`, `reschedule()`, `assignTherapist()`, `assignRoom()`, `addNote()`.

### 2. `TherapistSchedule` Aggregate
