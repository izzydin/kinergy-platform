...
/**
 * Use case handler orchestrating positive manual stock adjustments (surplus reconciliation).
    ...
    try {
      ...
      if (!reason || reason.length < 3) {
        return ApplicationResult.fail(
          'A valid reason (minimum 3 characters) is required for stock adjustment in.',
        );
      }
      if (typeof input.quantity !== 'number' || isNaN(input.quantity) || input.quantity <= 0) {
        return ApplicationResult.fail(
          'Adjustment quantity must be a positive number greater than zero.',
        );
      }
      ...
      // Domain mutation (generates StockMovement, increments version)
      ...
      const message = err instanceof Error ? err.message : 'Failed to adjust stock in.';
